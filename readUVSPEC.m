...
function [dataStruct,headers_units] = readUVSPEC(path,fileName,rte_solver)
...
if strcmp(rte_solver,'disort')==true
    ...
    headers_units{1,8} = 'totDwnIrrad';
    headers_units{2,8} = 'mW/(m^{2} nm)';
    ...
    dataStruct = struct(headers_units{1,1},data(:,1),headers_units{1,2},data(:,2),...
        ...
        headers_units{1,7},data(:,7),headers_units{1,8},data(:,8));
    
elseif strcmp(rte_solver,'twostr')==true
    
    headers_units = cell(2,6);
    headers_units{1,1} = 'wavelength';
    headers_units{1,2} = 'dirIrrad';
    headers_units{1,3} = 'difDwnIrrad';
    headers_units{1,4} = 'difUpIrrad';
